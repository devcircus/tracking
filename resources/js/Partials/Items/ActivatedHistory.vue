<template>
    <div class="flex flex-col">
        <div class="w-full bg-blue-800 p-4">
            <h1 class="text-white text-lg md:text-xl font-semibold uppercase">Recently Activated Tags</h1>
        </div>
        <vue-good-table ref="table" class="mb-8" :columns="tagColumns" :rows="rows" :pagination-options="paginationOptions" :search-options="searchOptions">
            <div slot="emptystate">
                No recently activated items found.
            </div>
        </vue-good-table>
    </div>
</template>
...
export default {
    ...
    data () {
        return {
            ...
            rows: this.item.recently_received_tags,
            paginationOptions: {
                enabled: true,
                mode: 'pages',
                perPage: 8,
