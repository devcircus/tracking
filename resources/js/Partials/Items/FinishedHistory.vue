<template>
    <div class="flex flex-col">
        <div class="w-full bg-blue-800 p-4">
            <h1 class="text-white text-lg md:text-xl font-semibold uppercase">Recently Finished Tags</h1>
        </div>
        <vue-good-table ref="table" class="mb-8" :columns="tagColumns" :rows="rows" :pagination-options="paginationOptions" :search-options="searchOptions">
            <div slot="emptystate">
                No recently finished items found.
            </div>
        </vue-good-table>
    </div>
</template>
...
export default {
    ...
    data () {
        return {
            ...
            rows: this.item.recently_finished_tags,
            paginationOptions: {
                enabled: true,
                mode: 'pages',
                perPage: 8,
