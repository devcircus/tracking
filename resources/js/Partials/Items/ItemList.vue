        ...
        <div>
            <vue-good-table ref="table" class="mb-8" :columns="itemColumns" :rows="rows" :pagination-options="itemPaginationOptions" :search-options="itemSearchOptions" :sort-options="itemSortOptions">
                ...
                <div slot="table-actions" class="flex items-center justify-between">
                    ...
                    <dropdown v-if="can('administerItems')" class="mr-1" right="10" width="180">
                        ...
                        <div slot="dropdown" class="flex flex-col mt-2 pt-2 shadow-lg bg-white rounded">
                            ...
                            <span v-if="can('createItems')" class="flex px-3 py-2 text-gray-700 hover:bg-blue-500 hover:text-white group cursor-pointer" @click="newItem()">
                ...
                <template slot="table-row" slot-scope="props">
                    <span v-if="props.column.field == 'actions'" class="flex justify-between px-3">
                        <button v-if="props.row.deleted_at && can('restoreItems')" class="text-green-500 hover:underline" tabindex="-1" type="button" @click="restoreItem(props.row.id)">Restore</button>
                        <button v-else-if="props.row.deleted_at === null && can('deleteItems')" class="text-red-500 hover:underline" tabindex="-1" type="button" @click="destroyItem(props.row.id)">Delete</button>
                        <inertia-link v-if="can('administerItems')" :href="route('items.show', props.row.id)" class="btn btn-text py-0 text-blue-500 hover:underline" tabindex="-1">View</inertia-link>
                    </span>
