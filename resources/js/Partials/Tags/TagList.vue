                ...
                <div slot="table-actions" class="flex items-center justify-between">
                    ...
                    <dropdown class="mr-1" right="10" width="180">
                        ...
                        <div slot="dropdown" class="mt-2 p-2 shadow-lg bg-white rounded">
                            ...
                            <checkbox v-if="can('deleteTags')" v-model="showTrashed" class="mb-2" label="Include deleted: " :width="4" :height="4" :checked="showTrashed" @input="hideDropdown()" />
                        </div>
                    </dropdown>
                </div>
                <template slot="table-row" slot-scope="props">
                    <span v-if="props.column.field == 'actions'" class="flex justify-between px-3">
                        <button v-if="props.row.deleted_at && can('restoreTags')" class="text-green-500 hover:underline" tabindex="-1" type="button" @click="restoreTag(props.row.id)">Restore</button>
                        <button v-else-if="props.row.deleted_at === null && can('deleteTags')" class="text-red-500 hover:underline" tabindex="-1" type="button" @click="destroyTag(props.row.id)">Delete</button>
